...
import EmisDialog from "./components/EmisDialog";
import emisParser from "./utils/EmisParser";
import { agruniData, freeuniData } from "./constants/EmisData";
...
const useStyles = makeStyles((theme) => ({
  ...
  addButton: (props) => ({
    backgroundColor: props.isFreeuni
      ? theme.palette.freeuni.main
      : theme.palette.agruni.main,
    "&:hover": {
      backgroundColor: props.isFreeuni
        ? theme.palette.freeuni.light
        : theme.palette.agruni.light,
    },
  }),
  iconButton: (props) => ({
    backgroundColor: props.isFreeuni
...
function App() {
          ...
          {(provided) => (
            <AnimatedCard
              ...
              setSubjectName={(newValue) =>
                updateCardField(index, "subjectName", newValue)
              }
              subjectCredits={card.subjectCredits}
              setSubjectCredits={(newValue) =>
                updateCardField(index, "subjectCredits", newValue)
              }
              subjectMark={card.subjectMark}
              isFreeuni={isFreeuni}
              setSubjectMark={(newValue) =>
                updateCardField(index, "subjectMark", newValue)
              }
  ...
  const searchChangeHandler = (event, value, reason) => {
    ...
    addCard(
      value.name,
      isFreeuni ? freeuniData[value.name] : agruniData[value.name],
      "",
      false
    );
  };
